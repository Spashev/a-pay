...
from accounts import UserAccountType
from accounts.tasks import send_created_account_notification
...
class UserManager(BaseUserManager):
    ...
    def create_superuser(self, email, password, **extra_fields):
        ...
        if extra_fields.get('is_staff') is not True:
            raise ValueError(_('accounts.custom_user_manager.value_error.not_staff'))
        if extra_fields.get('is_superuser') is not True:
            raise ValueError(_('accounts.custom_user_manager.value_error.not_superuser'))
